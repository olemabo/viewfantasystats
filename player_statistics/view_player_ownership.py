...
class PlayerOwnershipAPIView(APIView):
    def get(self, request, format=None):
        try:
            ...
            response = PlayerOwnershipApiResponse(
                empty_response, 
                current_gw, 
                all_gws, 
                team_names_and_ids_list, 
...
class PlayerOwnershipAPIView2(APIView):
    ...
    def get(self, request):
        ...
        chips_db_data = EliteserienChipsAndUserInfo.objects.all() if league_name == esf else PremierLeagueChipsAndUserInfo.objects.all()
        chip_data = [
            ChipModel(i.gw, i.extra_info_top_100 if league_name == esf else i.extra_info_top_10000, i.total_chip_usage_100 if league_name == esf else i.total_chip_usage_10000)
            for i in chips_db_data
        ]
