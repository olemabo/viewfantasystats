...
from constants import name_of_extra_info_file, name_of_nationality_file, path_to_store_local_data, web_global_league, all_top_x_players, time_to_sleep_for_each_iteration, name_of_ownership_file
import pandas as pd
import requests
import numpy as np
...
def data_fetch(num):
    """
    Will collect 50 teams from page "num" from global rank
    """
    time.sleep(0.4)
...
def get_player_name_ids():
    a = DataFetch()
    static_info = a.get_current_fpl_info()['elements']
...
def save_all_fpl_teams_stats():
    ...
    if not os.path.isdir(path_to_store_local_data + "/global_stats"):
        os.mkdir(path_to_store_local_data + "/global_stats/")

    # find max top_x players to check
    top_x_players = max(all_top_x_players)

    # get soccer player name and ids
    ids = get_player_name_ids()

    # create dict where each key is the soccer player id
    ids_dict = create_dict(ids, top_x_players)
    ...
    if not os.path.isdir(path_to_store_local_data + "/global_stats/" + str(last_gameweek)):
        os.mkdir(path_to_store_local_data + "/global_stats/" + str(last_gameweek))
    ...
    event_fpl_team_info = np.zeros((top_x_players, 3))
    dict_chip_to_index = {"freehit": 0, "bboost": 1, "3xc": 2, "wildcard": 3, None: 4}
    ...
    for player_i in tqdm(range(len(id_s))):
        ...
        if (player_i + 1) in all_top_x_players:
            ...
            path = path_to_store_local_data + "/global_stats/" + str(last_gameweek) + "/top_" + str(top_x_players_i)
            ...
            f2.write("Overall rank, team value, event transfers, event transfer cost\n")
            for n in range(top_x_players_i):
                f2.write(
                    str(n) + "," + str(event_fpl_team_info[n, 0]) + "," + str(event_fpl_team_info[n, 1]) + "," + str(
                        event_fpl_team_info[n, 2]) + "\n")
            f2.close()
