...
def find_best_rotation_combosEliteserien_gw_list(
        data, 
        gw_list, 
        teams_to_check=5, 
        teams_to_play=3, 
        team_ids=None,
        teams_in_solution=None, 
        teams_not_in_solution=None, 
    ...
    if teams_in_solution is None:
        teams_in_solution = []

    if teams_not_in_solution is None:
    ...
    if team_ids[0] != -1:
        if teams_to_check > len(team_ids):
    ...
    if not team_ids or team_ids[0] == -1:
    ...
    ids_must_be_in_solution = teams_in_solution

    # ids for the teams that cannot be in the solution
    ids_must_not_be_in_solution = teams_not_in_solution
    ...
    for team_combos in unique_team_ids:
        ...
        for GW_idx, GW in enumerate(gw_list):
            GW_home_scores_new = []
            GW_scores_new = []
            for team_idx, team_id in enumerate(team_combos):
                team_name = dict_with_team_ids_to_team_name[int(team_id)]
                temp_team_data_dict = create_FDR_dict(data_team_id_to_data[int(team_id)], 10)
                ...
                if gws_this_round > 1:
                    for i in range(gws_this_round):
                        ...
                        team_object_new.append(
                            FixtureDifficultyModel(
                                team_name=team_name,
                                opponent_team_name=data_gw[i][0].upper(),
                                difficulty_score=data_gw[i][2],
                                home_away=data_gw[i][1],
                                use_not_use=0,
                                message=data_gw[i][3]
                            ).to_dict())
                else:
                    ...
                    team_object_new.append(
                        FixtureDifficultyModel(
                            ...
                            difficulty_score=data_gw[0][2],
                            home_away=data_gw[0][1],
                            use_not_use=0,
                            message=data_gw[0][3]
                        ).to_dict())
            ...
            for k in Use_Not_Use_idx:
                two_D_list_new[k][GW_idx][0]["useNotUse"] = 1
        ...
        for team_id in team_combos:
            combo_names.append(dict_with_team_ids_to_team_name[int(team_id)])
        
        combos_with_score_new.append(
            [round(team_total_score_new / number_of_GW / teams_to_play, 4), 
             team_combos, 
             combo_names, 
             extra_fixtures,
             home_games, 
