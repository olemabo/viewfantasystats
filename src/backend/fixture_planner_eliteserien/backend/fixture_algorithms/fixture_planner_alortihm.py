...
def fdr_planner_eliteserien_fixture_list(fixture_list, gw_list):
    ...
    # Calculate FDR score for each team
    for fixture in fixture_list:
        fdr_dict = create_eliteserien_fdr_dict(fixture)
        fdr_score = calc_score_from_list(fdr_dict, gw_list)

        fdr_scores.append((fixture, fdr_score))

    # Sort teams by FDR score
    fdr_scores.sort(key=lambda x: x[1])

    for team_i, fdr_score in fdr_scores:
        temp_fixture_data = [[] for _ in gw_list]

        for j, gameweek in enumerate(team_i.gw):
            if gameweek not in gw_list:
                continue

            gw_index = gw_list.index(gameweek)

            fixture = FixtureDifficultyModel(
                team_name=team_i.team_name,
                opponent_team_name=team_i.oppTeamNameList[j],
                difficulty_score=team_i.oppTeamDifficultyScore[j],
                fdr_score=fdr_score,
                double_blank="",
                home_away=team_i.oppTeamHomeAwayList[j],
                use_not_use=0,
                message=team_i.messagesList[j],
            )

            temp_fixture_data[gw_index].append(fixture.to_dict())

        # Add blank fixture for gameweeks without a fixture
        for gw_index, gw_data in enumerate(temp_fixture_data):
            if not gw_data:
                temp_fixture_data[gw_index].append(
