...
export default function usePlayerOwnership(
  ...
  const [errorLoading, setErrorLoading] = useState<ErrorLoading>(
    emptyErrorLoadingState,
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const ownershipModels: PlayerOwnershipModel[] = data.ownershipdata.map(
          (item: string) => {
            ...
            return {
              ...
              ownership: getObjectDataFromKeys(
                parsed,
                data.newest_updated_gw,
                gwKeys,
              ),
              filter_out: false,
            };
