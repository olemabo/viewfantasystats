        ...
        <div className='chips-section'>
                ...
                <Table tableLayoutType={leagueType} className='chips-table'>
                    <TableHead>
                        ...
                        <TableCell cellType='head' minWidth={chipsRoundTableWidth}>
                            {playerOwnership("chip_title")} {general("round_short")}{currentGW}
                        </TableCell>
                        <TableCell cellType='head'>{playerOwnership("percent")}</TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        <TableRow>
                        <TableCell cellType='data' minWidth={chipsRoundTableWidth}>{playerOwnership("no_chip")}</TableCell>
                        <TableCell cellType='data'>{(chipUsageRound[4] / topXPlayers * 100).toFixed(1)}%</TableCell>
                        </TableRow>
                        <TableRow>
                        <TableCell cellType='data' minWidth={chipsRoundTableWidth}>{playerOwnership("wildcard")}</TableCell>
                        ...
                        <TableCell cellType='data' minWidth={chipsRoundTableWidth}>
                            {leagueType === LeagueTypes.ESF ? playerOwnership("rich_uncle") : playerOwnership("free_hit")}
                        </TableCell>
                        <TableCell cellType='data'>{(chipUsageRound[0] / topXPlayers * 100).toFixed(1)}%</TableCell>
                        </TableRow>
                        <TableRow>
                        <TableCell cellType='data' minWidth={chipsRoundTableWidth}>
                            {leagueType === LeagueTypes.ESF ? playerOwnership("forward_rush") : playerOwnership("bench_boost")}
                        </TableCell>
                        <TableCell cellType='data'>{(chipUsageRound[1] / topXPlayers * 100).toFixed(1)}%</TableCell>
                        </TableRow>
                        <TableRow>
                        <TableCell cellType='data' minWidth={chipsRoundTableWidth}>
                            {leagueType === LeagueTypes.ESF ? playerOwnership("two_captain") : playerOwnership("three_captain")}
                        </TableCell>
            ...
            {chipUsageTotal?.length > 3 &&
                <div>
                    <Table tableLayoutType={leagueType} className='chips-table'>
                        <TableHead>
                        <TableRow>
                            <TableCell cellType='head' minWidth={chipsAllRoundsTableWidth}>{playerOwnership("chip_total_usage_title")}</TableCell>
                            <TableCell cellType='head'>{playerOwnership("percent")}</TableCell>
                        </TableRow>
                        </TableHead>
                        <TableBody>
                        <TableRow>
                            <TableCell cellType='data'>{playerOwnership("wildcard")} nr. 1</TableCell>
                            <TableCell cellType='data'>{(chipUsageTotal[0] / topXPlayers * 100).toFixed(1)}%</TableCell>
                        </TableRow>
                        <TableRow>
                            <TableCell cellType='data'>{playerOwnership("wildcard")} nr. 2</TableCell>
                        ...
                        <TableRow>
                            ...
                            <TableCell cellType='data'>{(chipUsageTotal[leagueType === LeagueTypes.ESF ? 3 : 4] / topXPlayers * 100).toFixed(1)}%</TableCell>
                        </TableRow>
                        <TableRow>
                            <TableCell cellType='data'>
                            {leagueType === LeagueTypes.ESF ? playerOwnership("two_captain") : playerOwnership("three_captain")}
                            </TableCell>
                            <TableCell cellType='data'>{(chipUsageTotal[leagueType === LeagueTypes.ESF ? 4 : 3] / topXPlayers * 100).toFixed(1)}%</TableCell>
                        </TableRow>
                        </TableBody>
                    </Table>
                </div>
            ...
            <div>
                <Table tableLayoutType={leagueType}>
                    <TableHead>
                        <TableRow>
                        <TableCell cellType='head' minWidth={chipsTeamInfoTableWidthCol1}>{playerOwnership("team_info")}</TableCell>
                        <TableCell cellType='head' minWidth={chipsTeamInfoTableWidthCol2}>{playerOwnership("value")}</TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        <TableRow>
                        <TableCell cellType='data' minWidth={chipsTeamInfoTableWidthCol1}>{playerOwnership("avg_team_value")}</TableCell>
                        <TableCell cellType='data' minWidth={chipsTeamInfoTableWidthCol2}>{(chipUsageRound[5] / 10).toFixed(1)}</TableCell>
                        </TableRow>
                        <TableRow>
                        <TableCell cellType='data' minWidth={chipsTeamInfoTableWidthCol1}>{playerOwnership("avg_transfers")}</TableCell>
                        <TableCell cellType='data' minWidth={chipsTeamInfoTableWidthCol2}>{(chipUsageRound[6] / 10).toFixed(1)}</TableCell>
                        </TableRow>
                        <TableRow>
                        <TableCell cellType='data' minWidth={chipsTeamInfoTableWidthCol1}>{playerOwnership("avg_transfer_cost")}</TableCell>
