...
export default function PlayerOwnership({ 
    ...
    const { updatingGw, updatingPrecentage, topXPlayersList, availableGws, currentGW } = metadata;
    const { chipUsageRound, chipUsageTotal } = chip;

    const ownershipSorted = sortAndFilterPlayerOwnership(ownership, teamData, query, sortingKeyword, sortIndex, decreasing);
        ...
        <form className="form-stuff text-center">
            <div className='box-1'>
                ...
                >
                    ...
                    <option disabled style={{fontWeight: 900}}>{g("by_position")}</option>
                    <option value={SORTING_KEYWORDS.Goalkeepers}>{g("goalkeepers")}</option>
                    <option value={SORTING_KEYWORDS.Defenders}>{g("defenders")}</option>
                    <option value={SORTING_KEYWORDS.Midfielders}>{g("midfielders")}</option>
                    <option value={SORTING_KEYWORDS.Forwards}>{g("forwards")}</option>
                    <option disabled style={{fontWeight: 900}}>{g("by_team")}</option>
            ...
            <div className='box-3'>
                ...
                >
                    { availableGws.map(x => (
                        <option selected={x === currentGW} key={x} value={x}>{ x }</option>
                    ))}
                </select>
            </div>
            ...
            <>
                <div className="container-player-stats">
                    <Table tableLayoutType={leagueType} className='stat-table'>
                        <TableHead>
                            <TableRow>
                                <TableCell cellType='head' minWidth={139}>{t("player")}</TableCell>
                                ...
                                { leagueType === LeagueTypes.FPL && 
                                    <TableCell cellType='head'><TableSortHead text={t("three_captain")} reset={currentSorted != '3xC'} onclick={(increase: boolean) => sortOwnershipData(2, increase)}/></TableCell>
                                }
                                <TableCell cellType='head'>
                                    <TableSortHead
                                        text={t("vice_captain")}
                                ...
                                <TableCell cellType='head' className='last-element'>
                                    <TableSortHead 
                                        ...
                                        popover_text={p("topOwnership") + topXPlayers.toString() + ' ' + g("managers") + '.' } 
