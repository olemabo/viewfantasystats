        ...
        return (
            <Table tableLayoutType={leagueType}>
                <TableHead tableHeight="compact">
                    <TableRow>
                        <TableCell cellType="head" minWidth={playerNameMinWidth}>{g("player")}</TableCell>
                        ...
                        {hasOwnershipData && (
                            <TableCell cellType="head">
                                ...
                                >
                                    {p("moreInfoEO")}
                                    <a href={`/${leagueType === esf ? URLS.INTERNAL.ELITESERIEN.PLAYER_OWNERSHIP : URLS.INTERNAL.PREMIER_LEAGUE.PLAYER_OWNERSHIP}`}>{g("here")}</a>.
                                </Popover>
                            </TableCell>
                        )}
                    </TableRow>
            ...
            <Table tableLayoutType={leagueType} className="bonus-list">
                ...
                <TableBody>
                    {bonusList.map((bonus, index) => {
                        ...
                        return (
                            <TableRow key={`${bonus.home_player_name}-${bonus.away_player_name}-${index}`}>
                                <TableCell cellType="data" className="h">
                                    {bonus.home_opta > 0 ? `${bonus.home_player_name} (${bonus.home_opta.toFixed(0)})` : ''}
                                </TableCell>
                                <TableCell cellType="data" className="a">
                                    {bonus.away_opta > 0 ? `${bonus.away_player_name} (${bonus.away_opta.toFixed(0)})` : ''}
                                </TableCell>
                            </TableRow>
                        );
                    })}
