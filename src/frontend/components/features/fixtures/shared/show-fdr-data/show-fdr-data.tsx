...
export const ShowFDRData: FunctionComponent<ShowFDRProps> = ({
  ...
  return (
    <>
      {checkIfAllTeamsAreToggledOff() ? (
        <Message messageType="warning" messageText={warningMessage} />
      ) : (
              ...
              <table className="table-adjustment">
                <tbody>
                  <tr className="fdr-row-gws">
                    {kickOffTimes.map((gw) => (
                      <th key={gw.gameweek}>
                        {/* {languageContent.General.round_short} */}
                        {gw.gameweek}
                  ...
                  {fdrData.map((fdr, idx) => (
                    <>
                      {fdr.checked && (
                        <tr
                          key={"fdr-row-" + fdr.team_name}
                          id={"fdr-row-" + fdr.team_name}
                        >
                          {fdr.FDR.map((f) => (
                            <td
                              ...
                              className={
                                ...
                                (f.fdr_gw_i.length == 1
                                  ? " colors-" +
                                    f.fdr_gw_i[0].difficulty_score +
                                    " "
                                  : " no-padding ") +
                                "double-border-" +
                                f.fdr_gw_i[0].Use_Not_Use
                              }
                            >
                              {f.fdr_gw_i.map((g) => (
                                <div
                                  key={g.opponent_team_name + g.H_A}
                                  style={{
                                    backgroundColor: convertFDRtoHex(
                                      f.fdr_gw_i[0].difficulty_score,
                                      fdrToColor,
                                    ),
                                    position: g.message
                                  ...
                                  className={
                                    "color-" +
                                    Number(g.difficulty_score).toFixed(0) +
                                    " height-" +
                                    f.fdr_gw_i.length +
                                    (f.fdr_gw_i.length > 1
                                      ? " multiple-fixtures"
                                      : "") +
                                    " " +
                                    (f.fdr_gw_i[0].double_blank?.includes("-")
                                      ? "possible-blank"
                                      : "")
                                  }
                                >
                                  {g.message && (
                                    <Popover
                                      ...
                                      title=""
                                      // htmlTitle={languageContent.Fixture.uncertain_match}
                                      alignLeft={false}
                                      popoverTitle={""} // fdr.team_name + ' - ' + g.opponent_team_name
                                      iconSize={14}
                                      className={
                                        idx > fdrData?.length - 4
                                          ? "bottom-position"
                                          : ""
                                      }
