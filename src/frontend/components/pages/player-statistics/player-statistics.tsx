...
export function PlayerStatisticsPage({
  ...
  const playerStatisticsFiltered = sortAndFilterPlayerStatistics(
    playerStatistics,
    query,
    sortingTeamId,
    sortingPositionId,
  ...
  const totalNumberOfGwsList = Array.from(
    { length: totalNumberOfGws },
    (_, i) => i + 1,
  );
          ...
          {totalNumberOfGwsList.length > 0 && (
            <>
              ...
              <select
                ...
                name="sort_on"
              >
                <option key="First" selected={lastXGws == 0} value="0">
                  {g("total_all_gws")}
                </option>
                  ...
                  <>
                    ...
                    {categories.map((category, idx) => (
                      <TableCell
                        ...
                        className={
                          idx + 1 === categories.length ? "last-element" : ""
                        }
                      >
                        <TableSortHead
                ...
                {playerStatisticsFiltered
                  ...
                  .map((player_stat) => (
                    <TableRow key={player_stat.Name}>
                      ...
                      {player_stat?.player_statistics_list.map(
                        (stat, index) => (
                          <TableCell
                            key={`inner-${index.toString()}`}
                            cellType="data"
                            minWidth={getMinWidth(
                              categories[index],
                              lastXGws,
                          ...
                          >
                            <div>
                              {Number(stat).toFixed(
                                ["Mins"].includes(categories[index]) ? 0 : 1,
                              )}
                            </div>
                          </TableCell>
                        ),
