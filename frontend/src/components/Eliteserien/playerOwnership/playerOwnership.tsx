    ...
    return <>
     <div className='player-ownership-container' id="rotation-planner-container">
        ...
        <form className="form-stuff text-center">
            <div className='box-1'>
                ...
                <select onChange={(e) => filterOnPositionAndTeam(e.target.value, query)} className="input-box" id="sort_players_dropdown" name="sort_players">
                    <option selected={sorting_keyword == "All"} value="All">{content.all_players}</option>

                    <option disabled style={{fontWeight: 900}}>{content.by_position}</option>

                    <option selected={sorting_keyword == "Goalkeepers"} value="Goalkeepers">{content.goalkeeper}</option>
                    <option selected={sorting_keyword == "Defenders"} value="Defenders">{content.defenders}</option>
                    <option selected={sorting_keyword == "Midfielders"} value="Midfielders">{content.midfielders}</option>
                    <option selected={sorting_keyword == "Forwards"} value="Forwards">{content.forwards}</option>

                    <option disabled style={{fontWeight: 900}}>{content.by_team}</option>
            ...
            <div className='box-2'>
                <label>{content.top_x_managers}</label>
            ...
            <div className='box-5'>
                ...
                <input onChange={(e) => filterOnPositionAndTeam(sorting_keyword, e.target.value)} placeholder={content.search_text} className='input-box' type="search" id="site-search" name="q"></input>
            </div>
        ...
        <div className="container-player-stats">
            ...
            <thead>
                <tr>
                    <th className="name-col">{content.player}</th>
                    <th><TableSortHead popover_title='Effective Ownership' popover_text='Effektivt eierskap er en beregning som tar hensyn til managere som starter en spiller (ikke bare de som eier dem), sammen med de som kapteiner spilleren. Det er altså eierandel som starter spilleren pluss eierandelen som har kapteinet spilleren. ' text={content.eo} reset={currentSorted != 'EO'} defaultSortType={'Increasing'} onclick={(increase: boolean) => sortOwnershipData(0, increase)}/></th>
                    <th><TableSortHead popover_title='Valgt av' popover_text='Prosentandel som har denne spilleren i troppen sin (trenger ikke være i startelleveren).' text={content.owned_by} reset={currentSorted != 'Owned by'} defaultSortType={'Increasing'} onclick={(increase: boolean) => sortOwnershipData(4, increase)}/></th>
                    <th><TableSortHead text={content.captain} reset={currentSorted != 'Captaincy'} onclick={(increase: boolean) => sortOwnershipData(1, increase)}/></th>
                    <th><TableSortHead text={content.vice_captain} reset={currentSorted != 'VC'} onclick={(increase: boolean) => sortOwnershipData(3, increase)}/></th>
                    <th><TableSortHead text={content.benched} reset={currentSorted != 'Benched'} onclick={(increase: boolean) => sortOwnershipData(5, increase)}/></th>
                    <th className='last-element'><TableSortHead popover_title={content.tot_ownership} popover_text={'Prosentandel blant alle managere som eier denne spilleren. Altså ikke bare blant topp ' + topXPlayers.toString() + ' managere.' } text={content.tot_ownership} reset={currentSorted != 'Total Ownership'} onclick={(increase: boolean) => sortOwnershipData(6, increase)}/></th>
                </tr>
            </thead>
        ...
        { chipData?.length > 5 && chipData[0] != -1 && 
            <div className='chips-section'>
                <table className='chips-table'>
                    <thead>
                        <tr>
                            <th className="name-col-chips">{content.chip_title}</th>
                            <th>{content.percent}</th>
                        </tr>
                    </thead>

                    <tbody>
                        <tr>
                            <td className="name-col-chips"> <div className="format-name-col">{content.no_chip}</div> </td>
                            <td>{(chipData[4] / topXPlayers * 100).toFixed(1)} {'%'}</td>
                        </tr>
                        <tr>
                            <td className="name-col-chips"> <div className="format-name-col">{content.wildcard}</div> </td>
                            <td>{(chipData[3] / topXPlayers * 100).toFixed(1)} {'%'}</td>
                        </tr>
                        <tr>
                            <td className="name-col-chips"> <div className="format-name-col">{content.rich_uncle}</div> </td>
                            <td>{(chipData[0] / topXPlayers * 100).toFixed(1)} {'%'}</td>
                        </tr>
                        <tr>
                            <td className="name-col-chips"> <div className="format-name-col">{content.forward_rush}</div> </td>
                            <td>{(chipData[1] / topXPlayers * 100).toFixed(1)} {'%'}</td>
                        </tr>
                        <tr>
                            <td className="name-col-chips"> <div className="format-name-col">{content.two_captain}</div> </td>
                ...
                { totalChipUsage != null && totalChipUsage.length > 3 &&
                    <table className='chips-table'>
                        <thead>
                            <tr>
                                <th className="name-col-chips-2">{content.chip_total_usage_title}</th>
                                <th>{content.percent}</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            <tr>
                                <td className="name-col-chips-2"> <div className="format-name-col">{content.wildcard}{" nr. 1"}</div> </td>
                                <td>{(totalChipUsage[0] / topXPlayers * 100).toFixed(1)} {'%'}</td>
                            </tr>
                            <tr>
                                <td className="name-col-chips-2"> <div className="format-name-col">{content.wildcard}{" nr. 2"}</div> </td>
                                <td>{(totalChipUsage[1] / topXPlayers * 100).toFixed(1)} {'%'}</td>
                            </tr>
                            <tr>
                                <td className="name-col-chips-2"> <div className="format-name-col">{content.rich_uncle}</div> </td>
                                <td>{(totalChipUsage[2] / topXPlayers * 100).toFixed(1)} {'%'}</td>
                            </tr>
                            <tr>
                                <td className="name-col-chips-2"> <div className="format-name-col">{content.forward_rush}</div> </td>
                                <td>{(totalChipUsage[3] / topXPlayers * 100).toFixed(1)} {'%'}</td>
                            </tr>
                            <tr>
                                <td className="name-col-chips-2"> <div className="format-name-col">{content.two_captain}</div> </td>
                ...
                <table className='team-info-table'>
                    <thead>
                        <tr>
                            <th className="team-info">{content.team_info}</th>
                            <th>{content.value}</th>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        <tr>
                            <td className="team-info"> <div className="format-name-col">{content.avg_team_value}</div> </td>
                            <td>{(chipData[5]  / 10).toFixed(1)}</td>
                        </tr>
                        <tr>
                            <td className="team-info"> <div className="format-name-col">{content.avg_transfers}</div> </td>
                            <td>{(chipData[6] / 10).toFixed(1)}</td>
                        </tr>
                        <tr>
                            <td className="team-info"> <div className="format-name-col">{content.avg_transfer_cost}</div> </td>
